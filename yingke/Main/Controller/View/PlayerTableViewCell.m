...
{
    if (!_coverImage) {
        self.coverImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, _iconImage.frame.origin.y+_iconImage.frame.size.height +10, UIScreen_Width, (([UIScreen mainScreen].bounds.size.width ) * (Ratio)) + 1)];
    }
    return _coverImage;
}

- (void)setPlayerModel:(MiaowLiveModel *)playerModel
{
    ...
    // 用户Image
    [_iconImage sd_setImageWithURL:[NSURL URLWithString:playerModel.smallpic]];
    // 封面Image
    [_coverImage sd_setImageWithURL:[NSURL URLWithString:playerModel.bigpic] placeholderImage:[UIImage imageNamed:@"profile_user_414x414"]];
    
    // 观看人数
    _peopleNumber.text = [NSString stringWithFormat:@"%ld",playerModel.allnum];
