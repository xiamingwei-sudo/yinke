...
@property (strong, nonatomic) UITapImageView *userIconView, *userSexIconView, *coverView;
...
- (void)setBgImage:(UIImage *)bgImage{
    ...
    _bgImage = (bgImage != nil ? bgImage:[UIImage imageNamed:@"STARTIMAGE.jpg"]);
    [self updateData];
}
- (void)configUI{
    __weak typeof(self) weakSelf = self;
    if (!_coverView) {//遮罩
        _coverView = [[UITapImageView alloc] init];
        _coverView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.4];
        [self addSubview:_coverView];
        [_coverView addTapBlock:^(id obj) {
            if (weakSelf.bgImageClicked) {
                weakSelf.bgImageClicked();
            }
        }];
    ...
    [self setFrame:CGRectMake(0, 0, kScreen_Width, viewHeight +20)];
...
- (void)updateData{
    ...
    [self sd_setImageWithURL:[NSURL URLWithString:self.imageUrl] placeholderImage:[UIImage imageNamed:@"STARTIMAGE.jpg"]];
 
    [_userIconView sd_setImageWithURL:[@"http://pic.58pic.com/58pic/14/00/69/66858PICNfJ_1024.jpg" urlImageWithCodePathResize:2* _userIconViewWith] placeholderImage:kPlaceholderMonkeyRoundWidth(54.0)];
       //未知
    [_userSexIconView setImage:[UIImage imageNamed:@"n_sex_man_icon"]];
    _userSexIconView.hidden = NO;
    
    _userLabel.text = @"布卡与明太郎";
    [_userLabel sizeToFit];
    
    [_fansCountBtn setAttributedTitle:[self getStringWithTitle:@"粉丝" andValue:@"100"] forState:UIControlStateNormal];
    [_followsCountBtn setAttributedTitle:[self getStringWithTitle:@"关注" andValue:@"12"] forState:UIControlStateNormal];
